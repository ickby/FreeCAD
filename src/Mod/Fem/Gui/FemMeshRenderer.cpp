...
void FemMeshRenderer::buildBoundaryEdges()
{
    ...
    // The colouring asks an edge which cell it came from and nothing else, so
    // that is the one thing carried out with it.
    m_edgebuilder.setOriginArray(FemVisibilityMask::ArrayOrigCell);
    m_edgebuilder.build(surface, m_curvedmesh, m_realedges);
}
