...
/**
 * Stopwatch for the stages of the FEM view pipeline and the analysis
 ...
 *
 * It lives in the App library rather than beside the view because the work a
 * stage waits for is not all drawing: merging a chain, classifying its cells
 * and deriving the mesh topology are what the view asks of the document, and a
 * measurement that stopped at the library boundary would book all three as
 * time nothing accounts for.
 *
...
#define FEM_PERF_SCOPE(name) const Fem::PerfScope FEM_PERF_CONCAT(femPerfScope, __LINE__)(name)
